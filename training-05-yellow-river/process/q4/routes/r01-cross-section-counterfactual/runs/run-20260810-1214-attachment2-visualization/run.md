...
- 日期时间：2026-08-10 12:14（Asia/Shanghai）
- 执行目录：`F:\train`
- 执行命令：`python training-05-yellow-river/process/q4/routes/r01-cross-section-counterfactual/code/visualize_attachment2.py --input training-05-yellow-river/source/attachments/附件2.xlsx --run-dir training-05-yellow-river/process/q4/routes/r01-cross-section-counterfactual/runs/run-20260810-1214-attachment2-visualization --grid-step-m 5`
- 代码入口/版本：`process/q4/routes/r01-cross-section-counterfactual/code/visualize_attachment2.py`；执行前 SHA-256 `347DFC10EFB38EA8DE3C078BA10CF5B3759F35115B517E3EA9A73B6213860056`。
...
- 是否成功完成：no
- 异常与备注：运行在生成相邻测次变化汇总时失败；当前 NumPy 1.26.4 不提供 `np.trapezoid`，错误见 `logs/error.txt`。失败前写出的CSV属于不完整证据，不供引用。相邻测次变化图原计划为共同覆盖区上的探索性线性插值结果，尚不构成调水调沙因果结论。
