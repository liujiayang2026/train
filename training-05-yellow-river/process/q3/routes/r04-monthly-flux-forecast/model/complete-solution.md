...
- 对应问题：问题三（q3）
- 候选路线：`r04-monthly-flux-forecast`（由原重复编号 `r01-monthly-flux-forecast` 迁移）
- 当前依据运行：`run-20260809-2103-figure-label-clarity`
- 代码入口：`../code/monthly_flux_forecast.py`
- 主要目标：预测2022-2023年月水量、月输沙量及年度总量，给出月度预测区间，并据月度风险制定采样日期和时段。

> Provenance update（2026-08-09）：当前复现证据为 21:03 图示澄清运行，显式绑定 q1 20:32 与 q2 20:36 成功 run，数值与 20:40 运行逐字节一致。第 12 节 2/5/10 日规则只作为原始启发式基线保留；显式成本约束与动态监测优化见相邻路线 `r05-cost-constrained-sampling` 的 21:45 全局事件边界候选 run。该 run 在 21:24 连续动态日历基础上补齐跨月事件 ±1 日评分，尚待人工决定是否采用。
...
5. 模型选择和条件检验分离：2019—2020 扩展窗口回测用于选模，2021 一次性预测只作为 r04 预测模型层条件留出；上游 q1 补全可能使用全时段信息，因此不是端到端无泄漏检验。
...
主要训练数据来自问题一清洗后的不等间隔观测；以下路径相对训练包根目录：
...
该模型的优势是能够用少量参数平滑表达月度季节峰谷，同时岭惩罚可缓解48个月小样本下的共线性和过拟合。需要注意，实际训练只使用 2018 年后的样本，此时 \(T_t^{post}=T_t-2\)，两列在标准化后完全相同；岭惩罚使数值解唯一，但 \(\beta_1\) 与 \(\beta_2\) 不可分别解释，预测只依赖二者的合成趋势效应。综合论文写法见 r05 `model/complete-solution.md` 中的单一有效趋势变量 \(\tau_t=T_t-2\)，该写法不改变本路线代码和预测结果。
...
| 预测模型层条件留出 | 2018-2020 | 2021 | 不参与 r04 选模；上游并非端到端隔离 |
...
年度区间使用未四舍五入的月度端点逐月求和，用于风险范围展示，并非单独进行联合覆盖校准的年度置信区间。输沙量95%区间很宽，不是数值错误，而是2021异常残差在有限样本下对风险边界的真实影响。
...
- `R_width`：先计算 \((upper95-lower95)/\max(point,10^{-9})\)，再作极差归一化；
- `I_flood`：6-10月汛期指标；
- `I_peak`：7-10月峰值窗口指标；
- `R_abrupt`：问题二历史突变事件的月份频率。

权重把主要注意力放在“可能出现的高输沙上界”上，同时兼顾区间宽度和水文季节。上界和相对宽度均在 24 个未来月份上作极差归一化，若极差为 0 则统一映射为 0；风险分数的第 70 百分位按线性插值计算，达到该阈值者划为高风险，其余 4—11 月划为中风险，其他月份为低风险。
...
2022 年高风险月份为 7 月、10 月；2023 年高风险月份为 6—10 月。2022 年共 187 个采样时次，2023 年共 287 个，两年合计 474 个。该规则未包含成本目标与历史回放，现由 r05 将其作为对照基线；不得单独称为“最优采样”。

日尺度输出仅包含采样日期、采样时刻、风险等级、月风险分数和理由，不包含未来逐日水量或输沙量预测。

## 13. 可视化与图形解读

### 13.1 历史回测与 2021 预测模型层条件留出

![2019-2020选模回测与2021预测模型层条件留出](../runs/run-20260809-2103-figure-label-clarity/figures/historical_backtest.png)
...
![2022-2023月度预测及区间](../runs/run-20260809-2103-figure-label-clarity/figures/monthly_forecast_intervals.png)

蓝色虚线区分历史期与预测期。红线给出未来月度中心预测，深浅阴影表示80%和95%风险范围。2023区间比2022更宽，体现预测距离增加带来的不确定性。

### 13.3 月度采样强度

![2022—2023月度建议采样时次](../runs/run-20260809-2103-figure-label-clarity/figures/sampling_intensity.png)
...
3. 预测模型层按时间隔离：2019-2020选模，2021不参与 r04 选模；但上游 q1 补全可能使用全时段信息，因此不把该结果称为端到端无泄漏检验。
...
2. 代码中的两个趋势列在有效训练期完全共线，只能解释合成趋势效应，不能分别解释两个岭回归系数。
3. 2019回测只有2018一个完整训练年，复杂滞后模型处于冷启动状态。
4. 模型没有未来降雨、上游来水、水库调度和工程运行等外生变量，无法提前识别2021年8月式异常低谷。
5. 2023区间的1.25扩宽系数是基于预测步长的经验处理，应在论文中明确说明。
6. 年度区间由月度边界汇总，并非年度联合分布的严格置信区间。
7. 预测区间根据36个月历史外推残差校准，极端事件样本仍少，不能解释为绝对概率保证。

论文中应区分“历史拟合”和“真实提前预测”。本路线图中的2019-2021红线均为不使用测试年真实值的外推结果，不应为了追求视觉贴合而改成样本内拟合。

## 17. 最终结论

本候选预测路线采用“直接月积分 + 结构突变分段 + Fourier 岭回归 + 两阶段时间验证 + 经验预测区间”的方案。2019—2020 选模回测表明，`alpha=1` 的 Fourier 岭模型能够以较少参数描述突变后水沙序列的趋势和季节周期；2021 预测模型层条件留出揭示了无外生变量时对异常调度和季节错位的预测边界。当前复现给出 2022 年水量 458.39 亿 m³、输沙量 34164.05 万吨，2023 年水量 483.88 亿 m³、输沙量 37734.75 万吨，并通过 80% 和 95% 区间表达预测不确定性。原 474 时次风险规则只作为 r05 成本约束方案的历史对照；是否采用任何路线仍需人工决定。
