...
def train_tweedie_model(train_feat: pd.DataFrame) -> lgb.Booster:
...
def forecast_recursive_tweedie(
    model: lgb.Booster,
    ...
    for date in pd.date_range(start, end, freq="D"):
        ...
        one_day["日期"] = date
        one_day["货量"] = np.nan
        one_day = one_day[["场地1", "场地2", "edge", "日期", "货量"]]
        ...
        pred = model.predict(future_feat[features], num_iteration=model.best_iteration)
...
def main() -> None:
    ...
    print("\n训练Tweedie回测模型: 2021 -> 预测2022年1月")
    train_2021 = build_training_frame(panel, encoders, "2021-12-31")
    model_2021 = train_tweedie_model(train_2021)
    pred_2022 = forecast_recursive_tweedie(
        model_2021,
        panel[panel["日期"] <= "2021-12-31"].copy(),
        encoders,
        "2022-01-01",
        "2022-01-31",
    )
    actual_2022 = panel[(panel["日期"] >= "2022-01-01") & (panel["日期"] <= "2022-01-31")]
    metrics = metrics_frame(pred_2022, actual_2022)
    metrics.to_csv(RESULT_DIR / "problem1_lightgbm_tweedie_validation_metrics.csv", index=False, encoding="utf-8-sig")
    print("Tweedie回测指标:")
    print(metrics.round(4).to_string(index=False))

    print("\n训练最终Tweedie模型: 2021-2022 -> 预测2023年1月")
    train_full = build_training_frame(panel, encoders, "2022-12-31")
    model_full = train_tweedie_model(train_full)
    pred_2023 = forecast_recursive_tweedie(model_full, panel, encoders, "2023-01-01", "2023-01-31")
    save_outputs(pred_2023, "problem1_lightgbm_tweedie_2023_01_forecast")
